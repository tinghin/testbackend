const form1Service = require("../services/form1.services");
...
module.exports.getFormDetails = async (req, res) => {
  ...
  try {
    ...
    let json = await form1Service.getFormDetails(params, conn);
...
module.exports.insertFormDetails = async (req, res) => {
  ...
  try {
    ...
    let form_id = await form1Service.insertFormDetails(params, conn);
    await conn.doCommit();
    return res.send(
...
module.exports.updateFormStatus = async (req, res) => {
  ...
  try {
    ...
    await form1Service.updateFormStatus(params, conn);
    await conn.doCommit();
    return res.send(
...
module.exports.updateFormDetails = async (req, res) => {
  let conn;
  try {
    conn = await dbConnectUtils.init();
    let params = {
    ...
    await form1Service.updateFormDetails(params, conn);
    await conn.doCommit();
    return res.send(
      `Message: ${
        req.body.status === "Draft" ? "Save" : "Submit"
      } form successfully. Redirect to Homepage after 2 seconds`
    );
  } catch (err) {
    res.status(500).json({ message: "Error in invocation of API" });
    await conn.doRollback();
